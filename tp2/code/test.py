...
##################################################
    ...
    def drawBackButton(width, height):
        backButton.drawRectBorder(screen, darkGreen)
        backButton.draw(screen, black)  
        
    def drawPlayButton(screen):
        playButton.drawRectBorder(screen, darkGreen)
        playButton.draw(screen, black)        
    
    def drawCreateButton(screen):
        createButton.drawRectBorder(screen, darkGreen)
        createButton.draw(screen, black)
    
    def drawHelpButton(screen):
        helpButton.drawRectBorder(screen, darkGreen)
        helpButton.draw(screen, black)

    def drawButtons(screen):
        drawCreateButton(screen)
        drawPlayButton(screen)
        drawHelpButton(screen)
        
    def drawTitle(screen, text):
...
################################################## 
    ...
    def drawInstructions(width, height):
...
################################################## 
    ...
    def allSetsKeyPressed(event):
        nonlocal currentScreen
        if (pygame.key.name(event.key) == 'right'): #go to Set Screen
            currentScreen = 'setScreen'
            drawSetScreen(width, height) 
        elif (pygame.key.name(event.key) == 'left'): #go back to homeScreen
            currentScreen = 'homeScreen'
            drawHomeScreen()
        elif (pygame.key.name(event.key) == 'd'): #delete last set
            Set.deleteLastSet()
            drawAllSets(width, height)
...
################################################## 
    ...
    def drawCreateFormationButton(width, height):
        ...
        createFormationButton.draw(screen, black)
    ...
    def setScreenKeyPressed(event):
        nonlocal currentScreen
        if (pygame.key.name(event.key) == 'right'): #go to formations screen
            currentScreen = 'formationsScreen'
            drawFormationsScreen(width, height)  
        elif (pygame.key.name(event.key) == 's'): #save set
            marchers = copy.copy(Marcher.marchers)
            set = Set(marchers)
            currentScreen = 'allSets'
            drawAllSets(width, height)
        elif (pygame.key.name(event.key) == 'd'): #delete set
            currentScreen = 'allSets'
            drawAllSets(width, height)            
...
##################################################
    ...
    def formationsScreenKeyPressed(event):
        nonlocal formationNumber, currentScreen
        if (pygame.key.name(event.key) == 'right'): #creates formation and adds to formationList
            currentScreen = 'createFormation'
            formations.append([])
            formationNumber += 1
            drawCreateFormation(width, height)
        elif (pygame.key.name(event.key) == 'left'): #TODO: should it go back to allSets??? #goes back to setScreen 
            currentScreen = 'setScreen'
            drawSetScreen(width, height) 
        elif (chr(event.key) == 'd'): #deletes last formation
            if (formationNumber > 0):
                formations.pop(formationNumber-1)
                formationNumber -= 1
                currentScreen = 'formationsScreen' #TODO: delete formation without drawing new screen
...
##################################################
    ...
    def createFormationMousePressed(event): #creates a dot
        ...
        if (saveButton.hasClicked(pygame.mouse.get_pos())): #saves formation
            ...
            (dotX, dotY) = pygame.mouse.get_pos()
            addDot(dotX, dotY)
...
##################################################   
    ...
    def playScreenKeyPressed(event):
        nonlocal currentScreen
        if (pygame.key.name(event.key) == 'left'): #go back to home screen
            currentScreen = 'homeScreen'
            drawHomeScreen() 
        elif (pygame.key.name(event.key) == 'p'): #runs animation
            runPlayScreen() 
